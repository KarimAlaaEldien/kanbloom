...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Welcome back! Your garden is waiting 🌿");
      router.push("/dashboard");
    } catch (err: any) {
      console.error(err);
      let message = "Could not log in. Check your credentials.";
  ...
  return (
    <div className="w-full max-w-md p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-xl border border-neutral-200/50 dark:border-neutral-800/80 animate-pop-in">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <h2 className="font-heading text-2xl font-bold text-text-primary dark:text-white">Welcome Back</h2>
        <p className="text-sm text-text-secondary dark:text-neutral-400 mt-1.5 text-center">
          Log in to check on your growing tasks. Let's get back to cultivating! 🌿
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="block text-xs font-semibold uppercase tracking-wider text-text-secondary dark:text-neutral-400">
              Password
            </label>
          </div>
          <div className="relative">
      ...
      <p className="text-center text-xs text-text-secondary dark:text-neutral-400 mt-6">
        New to Kanbloom?{" "}
        <Link href="/signup" className="text-bloom-green hover:underline font-semibold">
          Plant your account here 🌱
        </Link>
      </p>
    </div>
  );
